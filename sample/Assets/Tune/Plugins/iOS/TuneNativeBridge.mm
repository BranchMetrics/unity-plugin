...
const char * UNITY_SENDMESSAGE_CALLBACK_TUNE_ENQUEUED_URL = "trackerDidEnqueueUrl";
...
{
    NSLog(@"Native: TuneSdkDelegate: enqueued action with reference id");
...
#pragma mark - In-App Marketing Methods
    ...
    // Push API
    bool TuneDidSessionStartFromTunePush()
    {
        return [Tune didSessionStartFromTunePush];
    }

    const char* TuneGetTuneCampaignIdForSession()
    {
        const char* campaignId;
        TunePushInfo *pushInfo = [Tune getTunePushInfoForSession];
        if (pushInfo) {
            campaignId = TuneAutonomousStringCopy([[Tune getTunePushInfoForSession].campaignId UTF8String]);
        }
        return campaignId;
    }

    const char* TuneGetTunePushIdForSession()
    {
        const char* pushId;
        TunePushInfo *pushInfo = [Tune getTunePushInfoForSession];
        if (pushInfo) {
            pushId = TuneAutonomousStringCopy([[Tune getTunePushInfoForSession].pushId UTF8String]);
        }
        return pushId;
    }
    ...
    bool TuneIsUserInSegmentId(const char* segmentId)
    {
        return [Tune isUserInSegmentId:TuneCreateNSString(segmentId)];
    }

    bool TuneIsUserInAnySegmentIds(const char* segmentIds[], int segmentCount)
    {
        // Convert array to NSArray
        NSMutableArray *segmentIdsArray = [NSMutableArray new];
        for (int i = 0; i < segmentCount; i++) {
            [segmentIdsArray addObject:TuneCreateNSString(segmentIds[i])];
        }
        return [Tune isUserInAnySegmentIds:segmentIdsArray];
    }

    void TuneForceSetUserInSegmentId(const char* segmentId, bool isInSegment)
    {
        [TuneDebugUtilities forceSetUserInSegment:isInSegment forSegmentId:TuneCreateNSString(segmentId)];
    }
